...
{
public:
  Student(): serviceTime(0), arrivalTime(0) {}
  Student(int t, int a): serviceTime(t), arrivalTime(a) {}
...
//student's service time is 1 to 10 min
void newArrivals(queue<Student>& line, int& studentCount, const int timePassed)
{
  srand(time(0));
  int arrived = (rand() % 6);
  ...
  {
    Student s = Student((rand() % 10 + 1), timePassed);
    line.push(s);
    studentCount++;
  }
}
...
void helpStudent(queue<Student>& line, int& timePassed, int*& waitTimes, int& index)
{
  waitTimes[index] = timePassed - (line.front()).getArrivalTime();
  index++;
  timePassed += (line.front()).getServiceTime();
  line.pop();
}
...
void runOfficeHours(const int officeTime, double& avgWaitTime, double& spentWithProf, double& overtime)
{
  ...
  Student firstStudent((rand() % 10 + 1), 0);
  studentCount++;
