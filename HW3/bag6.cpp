//NOTE: I CHANGED THE FILE TYPE FROM bag6.template TO bag6.cpp SO YOU MAY NEED TO CHANGE THIS IN THE HEADER FILE AS WELL
...
// HOMEWORK
	...
	// of the target were removed.
	{
      ...
      //Continue looking in left subtree
	    if (target < root_ptr->data( ))
	    {
	       bst_remove(root_ptr -> left(), target);
	    }
      //Continue looking in right subtree
	    if (target > root_ptr->data( ))
	    {
	       bst_remove(root_ptr -> right(), target);
	    }
      //Target  found and there is no left subtree, so remove this node
      //Right child is now the new root
	    if (root_ptr->left( ) == NULL)
	    {
		      oldroot_ptr = root_ptr;
		      root_ptr = root_ptr->right( );
      ...
      //move down tree and add new leaf
	    else
	    {
          ...
          while(!finished)
          {
            ...
            {
              //Check if this is largest node of the subtree
